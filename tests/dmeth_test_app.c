 ...
 *        through the real device node dmdevfs exposes for dmeth.
 *
 * Usage:
 *   dmeth_test_app <path-to-dmeth-device>
 *   e.g. dmeth_test_app /dev/dmeth0
 *
 * Unlike tests/dmeth_test.c (which calls dmeth_port_* directly with a
 * hand-built config, bypassing dmeth core/dmdrvi/dmdevfs entirely - see the
 * comment there and in tests/CMakeLists.txt), this opens the path with the
 * ordinary VFS file interface (open/ioctl/write/read/close) and drives it
 * purely through dmdrvi_ioctl() commands, exercising the real production
 * path end to end: dmdevfs must already have mounted and configured dmeth
 * (i.e. the board's own eth0.ini, see configs/board/) for this path to
 * exist at all. A pass here means the board's actual shipped configuration
 * (phy_address, buffer counts, RMII pin mux, ...) - not a synthetic one -
 * produces a working MAC and produces a working PHY.
 */

#define DMETH_TEST_APP_FRAME_BYTES    64U
...
static bool run_loopback_roundtrip(void *handle, dmeth_loopback_mode_t mode, const char *mode_name)
{
    ...
    uint8_t tx_frame[DMETH_TEST_APP_FRAME_BYTES];
    ...
    uint8_t rx_frame[DMETH_TEST_APP_FRAME_BYTES] = {0};
    if (ok)
    {
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        Dmod_Printf("Usage: dmeth_test_app <path-to-dmeth-device>\n");
        Dmod_Printf("e.g. dmeth_test_app /dev/dmeth0\n\n");
