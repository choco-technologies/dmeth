# Adding a New MCU Port to dmeth

`dmeth_port` currently supports `stm32f7`. The port is split from
the core `dmeth` module so a new architecture can be added without
...
2. Create `src/port/<family>/port.c` implementing the
   `dmod_dmeth_port_api_declaration(...)` functions declared in
   `include/dmeth_port.h`, plus `dmod_init`/`dmod_deinit` and any
