...
| `DMETH_IOCTL_SET_LOOPBACK_MODE`            | in               | `const dmeth_loopback_mode_t*` | Test-only: enable MAC/PHY internal loopback (must be set before `DMDRVI_IOCTL_NET_START`) - see `tests/dmeth_test_app.c` |
